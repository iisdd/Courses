...
def show_tfidf(tfidf, vocab, filename):                                                         # tfidf是矩阵,vocab是字符串为元素的列表
    # [n_doc, n_vocab]
    plt.imshow(tfidf, cmap="YlGn", vmin=tfidf.min(), vmax=tfidf.max())
    # plt.xticks(长度, 标注名)
    plt.xticks(np.arange(tfidf.shape[1]), vocab, fontsize=6, rotation=90)                       # 横坐标对应tfidf的列,也就是总单词数,内容是词汇
    plt.yticks(np.arange(tfidf.shape[0]), np.arange(1, tfidf.shape[0]+1), fontsize=6)           # 纵坐标对应tfidf的行,也就是文本数量,内容是数字
    plt.tight_layout()
    plt.savefig("./visual/results/%s.png" % filename, format="png", dpi=1000)                   # dpi:每英寸像素点数,调整清晰度的
    plt.show()
...
def show_w2v_word_embedding(model, data: utils.Dataset, path):
    ...
    for i in range(data.num_word):
        ...
        try:    # 数字涂蓝色,字母涂红色
            ...
            c = "red"
        plt.text(word_emb[i, 0], word_emb[i, 1], s=data.i2v[i], color=c, weight="bold") # bold指字体的粗细(较粗)
