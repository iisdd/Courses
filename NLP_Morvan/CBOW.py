...
tf.enable_eager_execution()         # 开启即时模式,解决tensor没有numpy()的报错
from utils import process_w2v_data
from visual import show_w2v_word_embedding

# 语料库中,9即和数字混在一起,又跟字母混在一起
corpus = [
    # numbers
    "5 2 4 8 6 2 3 6 4",
    "4 8 5 6 9 5 5 6",
...
class CBOW(keras.Model):
    def __init__(self, v_dim, emb_dim):
        super().__init__()
        self.v_dim = v_dim
        self.embeddings = keras.layers.Embedding(                               # 把词汇转化成坐标
            input_dim=v_dim, output_dim=emb_dim,  # [n_vocab, emb_dim], 把所有30个字符编译成2维空间的向量
    ...
    def call(self, x, training=None, mask=None):
        ...
        o = tf.reduce_mean(o, axis=1)   # [n, emb_dim],对输入的文本求平均
        return o
    ...
    def step(self, x, y):
        ...
        return loss.numpy()     # 要加tf.enable_eager_execution(),不然会报错
...
if __name__ == "__main__":
    d = process_w2v_data(corpus, skip_window=2, method="cbow")          # 取前后两个词判断中间的词
    print('长度: ', d.num_word)                                         # 30
    m = CBOW(d.num_word, 2)                                             # 在2维平面上可视化词向量
    train(m, d)
