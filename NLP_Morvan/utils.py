...
class DateData:
    def __init__(self, n):
        ...
        self.date_cn = []
        self.date_en = []
        for timestamp in np.random.randint(143835585, 2043835585, n):   # '24/Jul/1974' ~ '07/Oct/2034' 随机n个数
            date = datetime.datetime.fromtimestamp(timestamp)
            self.date_cn.append(date.strftime("%y-%m-%d"))
            self.date_en.append(date.strftime("%d/%b/%Y"))      # Y:1991, b:Jan
        self.vocab = set(
            [str(i) for i in range(0, 10)] + ["-", "/", "<GO>", "<EOS>"] + [
                i.split("/")[1] for i in self.date_en])         # vocab: 数字+月份+符号
...
def process_w2v_data(corpus, skip_window=2, method="skip_gram"):
    ...
    for c in corpus:
        ...
        if method == "skip_gram":           # 中间一个点对应2*skip_window个两边的点,有2*skip_window个pair
            for i in range(len(w_idx)):
                for j in js:
                    if i + j < 0 or i + j >= len(w_idx):
                        continue
                    pairs.append((w_idx[i], w_idx[i + j]))  # (center, context) or (feature, target)
        elif method.lower() == "cbow":      # 一个pair,包括[2*skip_window个context, 和中间那个点]
    ...
    if method.lower() == "skip_gram":       # 1对1
        x, y = pairs[:, 0], pairs[:, 1]
    elif method.lower() == "cbow":          # 2n对1
        x, y = pairs[:, :-1], pairs[:, -1]
    else:
        raise ValueError
