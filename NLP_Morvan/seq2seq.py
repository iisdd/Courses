# Seq2Seq机器翻译: 年月日 -> 日月年 Ex:'98-02-26' -> '26/Feb/1998'
...
class Seq2Seq(keras.Model):
    def __init__(self, enc_v_dim, dec_v_dim, emb_dim, units, max_pred_len, start_token, end_token):
        ...
        self.decoder_train = tfa.seq2seq.BasicDecoder(
            ...
            sampler=tfa.seq2seq.sampler.TrainingSampler(),   # sampler for train,用正确的标签值训练
            output_layer=decoder_dense
        )
        # predict decoder
        self.decoder_eval = tfa.seq2seq.BasicDecoder(
            cell=self.decoder_cell,
            sampler=tfa.seq2seq.sampler.GreedyEmbeddingSampler(),       # sampler for predict,用上一时刻的输出预测下一时刻
            output_layer=decoder_dense
        )
    ...
    def inference(self, x):
        ...
        for l in range(self.max_pred_len):
            ...
            pred_id[:, l] = o.sample_id         # o是softmax的输出,是一个概率分布,done就是<EOS>
        return pred_id

    def train_logits(self, x, y, seq_len):
        s = self.encode(x)
        dec_in = y[:, :-1]   # ignore <EOS>
        dec_emb_in = self.dec_embeddings(dec_in)    # 把连续的文本一个个喂入RNN
        o, _, _ = self.decoder_train(dec_emb_in, s, sequence_length=seq_len)
        logits = o.rnn_output   # sample出文本输出
        return logits

    def step(self, x, y, seq_len):
        with tf.GradientTape() as tape:
            logits = self.train_logits(x, y, seq_len)
            dec_out = y[:, 1:]  # ignore <GO>, 有个起点<GO>
            loss = self.cross_entropy(dec_out, logits)
            grads = tape.gradient(loss, self.trainable_variables)
        self.opt.apply_gradients(zip(grads, self.trainable_variables))      # 类似pytorch的loss反向传递
        return loss.numpy()
...
def train():
    ...
    print("Chinese time order: yy/mm/dd ", data.date_cn[:3], "\nEnglish time order: dd/b/yyyy ", data.date_en[:3])      # b:Jan
    ...
    for t in range(1500):
        ...
        if t % 70 == 0:
            target = data.idx2str(by[0, 1:-1])      # 去掉<GO>和<EOS>
            pred = model.inference(bx[0:1])
            res = data.idx2str(pred[0])             # pred套了两层括号: [[1, 2, 3, 4, 5,...]], 所以用pred[0]
